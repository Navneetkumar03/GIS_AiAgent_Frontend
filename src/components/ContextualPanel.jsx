...
const ContextualPanel = ({
  ...
  zoneData = null,   // 👈 new prop for App2 zone data
}) => {
  const [subcategoryData, setSubcategoryData] = useState({})
  const [isLoadingSubcategories, setIsLoadingSubcategories] = useState(false)
  const [subcategoryError, setSubcategoryError] = useState('')

  // ── Determine if we are in App2 mode ──
  const isApp2 = !!zoneData && zoneData.zones?.length > 0

  // ── Clear selections when switching between App1 and App2 ──
  useEffect(() => {
    if (setSelectedSubcategories) {
      setSelectedSubcategories({})
    }
  }, [isApp2, setSelectedSubcategories])
  ...
  useEffect(() => {
    ...
    async function loadSubcategories() {
      ...
      try {
        // ── Choose POI source (App1 or App2) ──
        let pois = {}
        if (isApp2) {
          // Combine POIs from all zones
          zoneData.zones.forEach(zone => {
            if (zone.pois) {
              Object.entries(zone.pois).forEach(([category, items]) => {
                if (!pois[category]) pois[category] = []
                pois[category].push(...items)
              })
            }
          })
        } else {
  ...
  }, [mode, selectedCategories, poiData, zoneData, isApp2])
          ...
          <div className="min-h-0 flex-1 overflow-y-auto pr-1 custom-scroll">
            ...
            <div className="mb-4 border-t border-white/30 pt-4 pr-12">
              <p className="text-xs font-semibold uppercase tracking-widest text-slate-700">
                {isApp2 ? 'Zone Analysis' : 'Category Details'}
              </p>
            </div>
            ...
            <div className="flex flex-col gap-4 pb-4">
              ...
              {!isLoadingSubcategories && !subcategoryError && !selectedCategories.length && (
                <div className="rounded-xl bg-white/60 p-3 text-sm text-slate-700 shadow-sm">
                  {isApp2
                    ? 'Select categories to view zone insights.'
                    ...
                    >
                      ...
                      {items.length ? (
                        ...
                        <p className="mt-2 text-sm text-slate-600">
                          {isApp2
                            ? 'No subcategories found for this zone.'
              ...
              {/* Optional: App2 info message */}
              {isApp2 && !isLoadingSubcategories && !subcategoryError && selectedCategories.length === 0 && (
                <div className="rounded-xl bg-cyan-50/70 p-3 text-xs text-cyan-700 border border-cyan-200 shadow-sm">
                  💡 Use the chat panel to ask detailed questions about car showroom locations, infrastructure, and zone comparisons.
                </div>
              )}
            </div>
          </div>
