...
function excludeShowroom(summary) {
  ...
  return Object.fromEntries(
    Object.entries(summary).filter(
      ([key]) => normalizeKey(key) !== "carshowroom",
    ),
  );
}
...
const CityWiseAnalyse = ({
  ...
  function handleSelectAll() {
    setSelectedZones(
      new Set(secondDropdownItems.map(getZoneValue).map(String)),
    );
  }
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      if (onZonesSelected) {
        ...
        console.log("SELECTED ZONES =", selectedLabels);
      ...
      if (onApp2SessionReady) {
        try {
          const initData = await analyzeZonesWithLLM(
            selectedItem,
            result.zones,
          );

          console.log("APP2 INIT DATA =", initData);
          ...
          if (initData?.session_id) {
            ...
            console.log("SESSION PASSED TO APP =", initData.session_id);
          } else {
            console.error("NO SESSION ID RETURNED");
          }
        } catch (err) {
          console.error("App2 LLM initialisation failed:", err);
        }
      }
    } catch (error) {
      console.error(error);
  ...
  function handleCategoryClick(zoneName, category) {
    const zone = analysisResult?.zones?.find((z) => z.zone === zoneName);
    ...
    onCategorySelect?.(category, pois, zoneName);
  }
    ...
    return (
      <div className="grid grid-cols-2 gap-1.5">
        {entries.map(([label, count]) => {
          const isActive = selectedZoneLayers.some(
            (item) => item.zoneName === zoneName && item.category === label,
          );
...
`}
  ...
  function getTotalRegistrations(regSummary) {
    ...
    return (
      regSummary.total || regSummary.registrations || regSummary.count || 0
    );
  }
  ...
  const totalVehicles = hasZones
    ? zones.reduce(
        (sum, zone) =>
          sum + getTotalRegistrations(zone.vehicle_registration_summary),
        0,
      )
    : 0;
  ...
  const combinedInfra = hasZones
    ? zones.reduce((acc, zone) => {
        Object.entries(excludeShowroom(zone.summary)).forEach(
          ([key, count]) => {
  ...
  const hasRoadData = zones.some(
    (z) => z.road_summary && Object.keys(z.road_summary).length > 0,
  );
    ...
    <div className="rounded-xl bg-white/72 p-2 shadow-[0_18px_50px_rgba(15,23,42,0.08)] backdrop-blur-xl">
      ...
      <div className="mb-1 flex items-center justify-between">
        ...
        <div className="group relative inline-block">
          <button
            type="button"
            onClick={onBack}
            className="group rounded-full bg-[#95cac7] p-1 text-sm font-semibold text-white transition hover:bg-cyan-700"
          >
            <img
              src={targetIcon}
              alt="Back"
        ...
        <div className="relative">
          ...
          >
            {dropdownItems.map((item, i) => (
              <option key={item} value={item} disabled={i === 0}>
                {item}
              </option>
            ))}
          </select>
      ...
      {citySelected && (
        <div className="mt-3">
          ...
          <div className="mt-3">
            ...
            >
              ...
              <span>
                {isAnalyzing ? "Analyzing..." : "Analyze Selected Zones"}
              </span>
            </button>
          </div>
          ...
          {hasZones && (
            <div className="mt-4 space-y-5">
              ...
              <div className="rounded-xl bg-gradient-to-r from-cyan-50 to-blue-50 border border-cyan-200 p-3 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  <div className="rounded-xl bg-white/80 px-2 py-2 shadow-sm border border-slate-200 overflow-hidden">
                    <p className="text-[9px] text-slate-500 uppercase truncate font-extrabold">
                      Total Car Dealers
                    </p>
              ...
              {zones.map((zone, idx) => {
                ...
                const regCount = getTotalRegistrations(
                  zone.vehicle_registration_summary,
                );

                const infraSummary = excludeShowroom(zone.summary);

                const isShowroomActive = selectedZoneLayers.some(
                  (item) =>
                    item.zoneName === zoneName &&
                    item.category === "Car Showroom",
                );
                const showroomIcon = iconMap["carshowroom"] || "";
                return (
                  <div
                    key={zoneName}
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                      ...
                      {showroomCount > 0 && (
                        <div
                          onClick={() =>
                            handleCategoryClick(zoneName, "Car Showroom")
                          }
                          className={`
            flex flex-col items-center justify-center
            rounded-lg px-2 py-1.5 shadow-sm border cursor-pointer
        ...
        `}
                        >
                          ...
                          <span
                            className={`text-sm font-bold ${
                              isShowroomActive ? "text-white" : "text-cyan-900"
                            }`}
                          >
                            {showroomCount}
                          </span>
                      ...
                      {Object.entries(infraSummary).map(([label, count]) => {
                        const isActive = selectedZoneLayers.some(
                          (item) =>
                            item.zoneName === zoneName &&
                            item.category === label,
